...
const ALL_ROLES = ['ADMIN', 'KITCHEN', 'CENTERS', 'STORE', 'COO', 'RESORT', 'AGGREGATE', 'PARTNER', 'RESTAURANT', 'FINANCE', 'HR'];
...
const EntityDetail: React.FC = () => {
  ...
  const fetchData = async () => {
    ...
    try {
      setIsLoading(true);
      const [entitiesRes, adminRes] = await Promise.all([
        entityApi.getAll(),
        entityApi.getAdmins(id)
      ]);
      const found = entitiesRes.data.data.find((e: any) => e._id === id);
      setEntity(found);
  ...
  const handleAddUser = async (e: React.FormEvent) => {
    ...
    try {
      ...
      if (newUser._id && !payload.password) delete payload.password;
      if (customLicenseDate) payload.customLicenseDate = customLicenseDate;

      if (newUser._id) {
        await userApi.update(newUser._id, payload);
      } else {
        await entityApi.addAdmin(id, payload);
      }
  ...
  const handleRenew = async (e: React.FormEvent) => {
    ...
    if (!window.confirm(`Confirm renewal for ${selectedUser.name}?`)) return;
    try {
      setIsSubmitting(true);
      await paymentApi.manualRenewal({
        ...
        ...(renewalData.customLicenseDate ? { customLicenseDate: renewalData.customLicenseDate } : {})
      } as any);
      setIsRenewing(false);
      setSelectedUser(null);
      setRenewalData({ amount: 0, duration: 1, customLicenseDate: '' } as any);
  ...
  const handleEditEntity = async (e: React.FormEvent) => {
    ...
    try {
      setIsSubmitting(true);
      setError('');
      await entityApi.update(id, editEntityData);
  ...
  return (
    <MainLayout>
      ...
      <header className="entity-header">
        ...
        <div className="header-actions">
          <button className="btn-secondary" onClick={() => {
            setEditEntityData({ name: entity.name, location: entity.location, username: entity.username });
        ...
        <div className="table-wrapper">
          <table className="sharp-table">
            ...
            <tbody>
              ...
              {admins.length === 0 && <tr><td colSpan={6} className="empty-row">NO PERSONNEL REGISTERED</td></tr>}
            </tbody>
          </table>
        </div>
      </div>
          ...
          <div className="modal-content">
            ...
            <form onSubmit={handleRenew}>
              ...
              <div className="modal-actions">
                ...
                <button type="submit" className="btn-primary" disabled={isSubmitting}>{isSubmitting ? 'PROCESSING...' : 'CONFIRM PAYMENT'}</button>
              </div>
            </form>
          </div>
        </div>
      ...
      <style>{`
        ...
        .u-contact p { margin-bottom: 2px; font-size: 0.8rem; }
        ...
        .u-status.expired .dot { background: #ef4444; }
        ...
        .optional-label { font-weight: 400; font-size: 0.65rem; color: var(--text-dim); margin-left: 4px; }
      `}</style>
    </MainLayout>
  );
};
